...
def discover_python_files(repo_path):
    """Finds all .py files in the repository using git ls-files."""
    abs_repo_path = os.path.abspath(repo_path)
    try:
        # Ensure the command is run in the repository's directory
        ...
        py_files_relative = result.stdout.strip().split('\n')
        # Filter out any empty strings that might result from split
        py_files_relative = [f for f in py_files_relative if f]
        # Prepend the repo_path to make them absolute
        py_files_absolute = [os.path.join(abs_repo_path, f) for f in py_files_relative]
        return py_files_absolute
...
def get_git_file_history(file_path, repo_abs_path):
    """
    Retrieves the last commit timestamp and total commit count for a given file.
    """
    relative_file_path = os.path.relpath(file_path, repo_abs_path)
    timestamp = 0
    count = 0
    ...
    try:
        ...
        ts_result = subprocess.run(
            ['git', 'log', '-1', '--format=%ct', '--', relative_file_path],
            cwd=repo_abs_path,
            capture_output=True,
            text=True,
            check=False # Check manually to handle empty output
        )
        if ts_result.returncode == 0 and ts_result.stdout.strip():
            timestamp = int(ts_result.stdout.strip())
        else:
            # print(f"Warning: Could not get last commit timestamp for {relative_file_path}. Error: {ts_result.stderr}")
            pass
        ...
        if count_result.returncode == 0 and count_result.stdout.strip():
            ...
            # print(f"Warning: Could not get commit count for {relative_file_path}. Error: {count_result.stderr}")
            pass

    except subprocess.CalledProcessError as e: # Should not be reached if check=False
        # print(f"CalledProcessError while getting git history for {relative_file_path}: {e}")
        pass # timestamp and count remain 0
    except FileNotFoundError:
        # print("Error: git command not found. Cannot get file history.")
        pass # timestamp and count remain 0
    except ValueError: # Handle potential int conversion error if git output is unexpected
        # print(f"ValueError converting git output for {relative_file_path}.")
        pass # timestamp and count remain 0

    return {'last_commit_timestamp': timestamp, 'commit_count': count}
...
def analyze_repo(repo_path, external_modules_str, 
                 ...
                 weight_recency, weight_commit_count): # Updated signature
    
    # Hardcoded weights removed, now passed as parameters
    # weight_recency = 0.1 
    # weight_commit_count = 0.1
    ...
    file_git_history = {} # NEW: Initialize dictionary
    ...
    for f_path in all_py_files:
        ...
        # NEW: Get and store git history for the file
        history = get_git_file_history(f_path, abs_repo_path)
        file_git_history[f_path] = history
    ...
    # === Pre-calculation for Git History Normalization ===
    all_timestamps = [h['last_commit_timestamp'] for h in file_git_history.values() if h['last_commit_timestamp'] > 0]
    all_counts = [h['commit_count'] for h in file_git_history.values() if h['commit_count'] > 0]

    max_timestamp = max(all_timestamps) if all_timestamps else 0
    # min_timestamp = min(all_timestamps) if all_timestamps else 0 # For age calculation
    current_time_ts = time.time()

    max_commit_count = max(all_counts) if all_counts else 0

    print("\n--- CodeRank Results (including Git History) ---") # Update print statement
    code_ranks = {}
    for module_fqn in internal_module_fqns:
        pagerank_score_imported = pagerank_being_imported.get(module_fqn, 0)
        pagerank_score_importing = pagerank_importing_others.get(module_fqn, 0)
        current_pagerank_score = pagerank_score_imported + pagerank_score_importing

        recency_score_component = 0
        commit_count_score_component = 0

        f_path = module_map.get(module_fqn)
        if f_path and f_path in file_git_history:
            history = file_git_history[f_path]
            
            # Recency Score
            if max_timestamp > 0 and history['last_commit_timestamp'] > 0:
                # Normalize timestamp directly: more recent = higher value
                normalized_recency = history['last_commit_timestamp'] / max_timestamp
                recency_score_component = normalized_recency * weight_recency
            # Alternative: Age-based recency (older = lower score)
            # if history['last_commit_timestamp'] > 0:
            #    age_seconds = current_time_ts - history['last_commit_timestamp']
            #    oldest_age_seconds = current_time_ts - min_timestamp if min_timestamp > 0 and current_time_ts > min_timestamp else (current_time_ts if current_time_ts > 0 else 1)
            #    if oldest_age_seconds <=0: oldest_age_seconds = 1 # Avoid division by zero
            #    normalized_age = age_seconds / oldest_age_seconds
            #    recency_score_component = (1 - normalized_age) * weight_recency # Invert so less age = more score
            # else: # No timestamp, no recency score
            #    recency_score_component = 0


            # Commit Count Score
            if max_commit_count > 0 and history['commit_count'] > 0:
                normalized_commit_count = history['commit_count'] / max_commit_count
                commit_count_score_component = normalized_commit_count * weight_commit_count
        
        code_ranks[module_fqn] = current_pagerank_score + recency_score_component + commit_count_score_component
    ...
    if not sorted_ranks:
        ...
        if display_ranks_console:
            ...
            # Calculate max lengths for new columns
            console_max_commit_date_len = 10 # YYYY-MM-DD
            console_max_commit_count_len = 0
            for module_fqn, _ in display_ranks_console:
                f_path_hist_console = module_map.get(module_fqn)
                history_console = file_git_history.get(f_path_hist_console) if f_path_hist_console else None
                if history_console and history_console['commit_count'] > 0:
                    count_str_len = len(str(history_console['commit_count']))
                    if count_str_len > console_max_commit_count_len:
                        console_max_commit_count_len = count_str_len
            if console_max_commit_count_len < len("Commits"): # Ensure header fits
                console_max_commit_count_len = len("Commits")

        header_module_console = 'Module'
        header_score_console = 'CodeRank Score'
        header_last_commit_console = 'Last Commit'
        header_commit_count_console = 'Commits' # Shorter for console
        ...
        if console_max_commit_date_len < len(header_last_commit_console): # Should not happen with YYYY-MM-DD
            console_max_commit_date_len = len(header_last_commit_console)
        if console_max_commit_count_len < len(header_commit_count_console):
            console_max_commit_count_len = len(header_commit_count_console)
        ...
        if display_ranks_console:
            print(f"{header_module_console.ljust(console_max_name_len)} | {header_score_console.ljust(14)} | {header_last_commit_console.ljust(console_max_commit_date_len)} | {header_commit_count_console.ljust(console_max_commit_count_len)}")
            print(f"{'-' * console_max_name_len} | {'-' * 14} | {'-' * console_max_commit_date_len} | {'-' * console_max_commit_count_len}")
            for module_fqn, rank in display_ranks_console:
                f_path_console = module_map.get(module_fqn)
                history_console = file_git_history.get(f_path_console) if f_path_console else None
                commit_date_str_console = "N/A"
                if history_console and history_console['last_commit_timestamp'] > 0:
                    commit_date_str_console = datetime.fromtimestamp(history_console['last_commit_timestamp']).strftime('%Y-%m-%d')
                commit_count_str_console = "N/A"
                if history_console and history_console['commit_count'] > 0:
                   commit_count_str_console = str(history_console['commit_count'])

                print(f"{module_fqn.ljust(console_max_name_len)} | {rank:<14.6f} | {commit_date_str_console.ljust(console_max_commit_date_len)} | {commit_count_str_console.ljust(console_max_commit_count_len)}")
        ...
        try:
            with open(output_file_path, 'w', encoding='utf-8') as f_out:
                ...
                if not sorted_ranks:
                    ...
                    file_header_last_commit = 'Last Commit'
                    file_header_commit_count = 'Commit Count'
                    
                    file_max_name_len = 0
                    file_max_commit_date_len = 10 # YYYY-MM-DD
                    file_max_commit_count_len = 0
                    ...
                    if sorted_ranks: # Calculate max_name_len based on ALL ranks for file output
                        ...
                        for module_fqn, _ in sorted_ranks:
                            f_path_hist_file = module_map.get(module_fqn)
                            history_file = file_git_history.get(f_path_hist_file) if f_path_hist_file else None
                            if history_file and history_file['commit_count'] > 0:
                                count_str_len_file = len(str(history_file['commit_count']))
                                if count_str_len_file > file_max_commit_count_len:
                                    file_max_commit_count_len = count_str_len_file
                    
                    if file_max_name_len < len(file_header_module):
                        file_max_name_len = len(file_header_module)
                    if file_max_commit_date_len < len(file_header_last_commit):
                        file_max_commit_date_len = len(file_header_last_commit)
                    if file_max_commit_count_len < len(file_header_commit_count):
                         file_max_commit_count_len = len(file_header_commit_count)


                    f_out.write(f"{file_header_module.ljust(file_max_name_len)} | {file_header_score.ljust(14)} | {file_header_last_commit.ljust(file_max_commit_date_len)} | {file_header_commit_count.ljust(file_max_commit_count_len)}\n")
                    f_out.write(f"{'-' * file_max_name_len} | {'-' * 14} | {'-' * file_max_commit_date_len} | {'-' * file_max_commit_count_len}\n")
                    for module_fqn, rank in sorted_ranks: # Write all ranks to file
                        f_path_file = module_map.get(module_fqn)
                        history_file = file_git_history.get(f_path_file) if f_path_file else None
                        commit_date_str_file = "N/A"
                        if history_file and history_file['last_commit_timestamp'] > 0:
                            commit_date_str_file = datetime.fromtimestamp(history_file['last_commit_timestamp']).strftime('%Y-%m-%d')
                        commit_count_str_file = "N/A"
                        if history_file and history_file['commit_count'] > 0:
                            commit_count_str_file = str(history_file['commit_count'])
                        
                        f_out.write(f"{module_fqn.ljust(file_max_name_len)} | {rank:<14.6f} | {commit_date_str_file.ljust(file_max_commit_date_len)} | {commit_count_str_file.ljust(file_max_commit_count_len)}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Calculate CodeRank for Python files in a repository, incorporating Git history.") # Updated description
    ...
    # New arguments
    parser.add_argument("--weight_recency", type=float, default=0.1,
                        help="Weight for recency factor. (Default: 0.1)")
    parser.add_argument("--weight_commit_count", type=float, default=0.1,
                        help="Weight for commit count factor. (Default: 0.1)")
    ...
    analyze_repo(args.repo_path, args.external_modules,
                 ...
                 args.weight_recency, args.weight_commit_count) # Pass new args
